...
def internal_multi(df_int, i):
    df_int.drop(columns=['n_node'], inplace=True)
    df_int.set_index(['o', 'd', 'uc', 'route', 'abs_demand', 'pct_demand'], inplace=True)
    df_int['Nodes'] = df_int['Nodes'].str.split(',')
    df_int = df_int.join(pd.DataFrame(df_int['Nodes'].tolist(), index=df_int.index))
    drop_cols = ['Nodes', 0, i - 1, i]
    df_int.drop(columns=drop_cols, inplace=True)
    if len(df_int.columns) < 2:
        return
    a = df_int[range(1, i - 2)].stack().to_frame()
    b = df_int[range(2, i - 1)]
    b.columns -= 1
    b = b.stack().to_frame()
    a.columns = ['a']
    b.columns = ['b']
    links = a.join(b).reset_index()
    links.rename(columns={'level_6': 'order'}, inplace=True)
    links.set_index(['uc', 'o', 'd', 'route', 'order', 'a', 'b'], inplace=True)
    return links
def read_satpig(path_to_satpig):
    """
    ...
    for i in df['n_node'].unique():
        dfs.append((df[df['n_node'] == i].deepcopy(), i))
...
if __name__=="__main__":
    df = read_satpig(r"Y:\Carbon\QCR_Assignments\07.Noham_to_NoCarb\2018\RotherhamBase_i8c_2018_TS2_v107_SatPig_uc4.csv")

    # df = pd.read_parquet(r"E:\misc_scripts\format_tests\satpig.parquet")
    df = pd.read_hdf(r"E:\misc_scripts\format_tests\satpig.h5", key='test', mode='r')
